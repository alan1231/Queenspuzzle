...
export default class HelloWorldApp extends Component {
  ...
  render() {
    var Queenspuzzle = []
    let num = Math.floor(Math.random() * 92) + 1
    for (let i = 0; i < this.state.queenAry[num - 1].length; i++) {
      for (let v = 0; v < this.state.queenAry[num - 1][i].length; v++) {
        Queenspuzzle.push(
          <View style={{ width: 40, height: 40, justifyContent: 'center', alignItems: 'center', backgroundColor: 'red' }}>
            <Text style>{this.state.queenAry[num - 1][i][v]}</Text>
          </View>
        )
      }
    }
